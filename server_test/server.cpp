...
void*handleConnection(void* arg){
    ...
    int addrlen = sizeof(c->addr);
	char buffer[1024] = { 0 };
    // int valread = 0;
    // valread = read(c->fd, buffer, 1024);
	// printf("%s\n", buffer);
	// for(int i = 0;i<=100000000;++i){
    //     send(c->fd, hello, strlen(hello), 0);
	...
	// read content send by client and store it into buffer.
	{
		...
		if(strcmp(buffer,"exit") == 0){
			...
			upload_c(c->fd, &buffer[++loc]);
		}
		else if(strncmp(buffer,"$download",9) == 0){
			download_c(c->fd, &buffer[10]);
		}
		else
...

{
	...
	*/
	...
	std::cout<<"Connection established! \n";
        ...
        c->fd = client_fd;
        c->addr = address;
        pthread_create(&t1 , NULL, handleConnection , (void*)c);
